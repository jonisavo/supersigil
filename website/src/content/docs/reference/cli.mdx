...
Builds the full document graph and runs structural checks, plugin evidence discovery, and coverage analysis. When `--since` is supplied, terminal output may append a short affected-document hint, GitHub output can render an affected-doc review dashboard, and JSON includes an optional `affected_summary`.

| Flag | Description |
|------|-------------|
| `--since <ref>` | Git ref for affected-document checks |
