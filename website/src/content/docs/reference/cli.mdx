...
| `--detail <compact\|full>` | JSON detail level: `compact` omits evidence records on clean runs, `full` includes everything (default: `compact`) |
...
Writes the six bundled agent skills to disk: `feature-specification`, `feature-development`, `retroactive-specification`, `spec-driven-development`, `refactoring`, and `ci-review`. If skills already exist at the target path, they are overwritten with the version embedded in the current binary.
