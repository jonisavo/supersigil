...
Builds the full document graph and runs structural checks, plugin evidence discovery, and coverage analysis. When `--since` is supplied, terminal and markdown output may append an affected-document note, and JSON includes an optional `affected_summary`.
