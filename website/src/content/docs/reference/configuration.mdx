...
multiple_expected_children = "error"     # default
inline_example_without_lang = "error"    # default
code_ref_conflict = "warning"            # default
```
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `diagnostics` | `"lint"` \| `"verify"` | `"verify"` | Diagnostics tier on save. `lint` runs only parse + structural checks (fast). `verify` additionally runs graph rules, coverage checks, and plugin evidence (slower but more complete). |
