...
Use `--project <name>` with commands like `supersigil verify` and `supersigil ls` to target a specific project.
...
<Aside type="note">
  Hard errors (`broken_ref`, `duplicate_id`, `dependency_cycle`, `expression_attribute`, `unknown_component`, `missing_required_attribute`) cannot be suppressed or reconfigured.
</Aside>
...
Define custom components beyond the 10 built-ins:
...
| `parallelism` | integer | `1` | Number of examples to run concurrently |
