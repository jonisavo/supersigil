...
1. **Structural checks run first.** All rules except coverage are evaluated: broken refs, unknown components, missing attributes, and so on.
...
3. **If no structural errors, examples execute.** Examples run concurrently (configurable parallelism, default 1). Each example gets a fresh temp directory, optional setup script, runner execution, and output matching.
...
**List examples:**
```bash
supersigil examples                  # All examples
