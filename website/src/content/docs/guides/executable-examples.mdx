...
The `<Example>` component embeds a runnable code sample in any spec document. Code content can be provided inline (for trivial cases) or in a separate code fence linked via `supersigil-ref` (for content needing syntax highlighting or XML-special characters):

````md
```sh supersigil-ref=greeting
supersigil --version
```
...
The code fence with `supersigil-ref=greeting` is linked to the Example by ID. When the example runs, its output is compared against the `<Expected>` declaration. If the example has a `verifies` attribute, a passing result produces verification evidence that counts toward criterion coverage.
...
| `lang` | no | Language of the code block. If omitted, derived from the `supersigil-ref` fence's language tag. Required when using inline text content |
...
`<Expected>` is a child of `<Example>` that declares the golden output. When the example runs, its output is compared against this expectation. Like `<Example>`, its code content can be inline or in a `supersigil-ref` fence:
...
**`snapshot`** -- Same as text matching on normal runs. When `--update-snapshots` is passed, a mismatch rewrites the expected body in the source file instead of failing.
...
1. **Start with a requirement that has a criterion**
   ...
   ````md title="specs/cli/req/help.md"
   ---
   supersigil:
     id: cli/req/help
     type: requirement
...
2. **Add an example that verifies the criterion**
   ...
   `````md title="specs/cli/req/help.md" ins={4-14}
   ```

   ## Examples

   ```sh supersigil-ref=version-check
   supersigil --version
   ```
