...
**`orphan_decision`** — A `<Decision>` has no outward connections (no `<References>`, `<TrackedFiles>`, or `<DependsOn>` children) and is not referenced by any other component. An orphan decision is disconnected from the graph.
...
When a decision is superseded, use `<References>` to link to the replacement, creating an audit trail:

```mdx
---
supersigil:
  id: arch/adr-old-auth
  type: adr
  status: superseded
---

<References refs="arch/adr-new-auth" />
```
