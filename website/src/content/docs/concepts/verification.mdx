...
| `multiple_expected_children` | error | `<Example>` has more than one `<Expected>` child |
| `inline_example_without_lang` | error | `<Example>` with inline code has no `lang` attribute and no fenced code block language |
| `code_ref_conflict` | warning | Orphan, duplicate, or dual-source `supersigil-ref` fence detected |
