...
Verification is what makes Supersigil more than documentation. It is a graph checker that catches broken references, uncovered criteria, missing tests, and impacted review scope -- automatically, on every commit.

## What Verify Does

`supersigil verify` builds the full document graph and checks everything: cross-document references, coverage, test mappings, affected-document hints, and cycles. This is the CI command.
...
A criterion is covered when the graph can resolve at least one evidence source for it -- explicit `<VerifiedBy>` mappings, ecosystem plugin discovery, or both. All sources are merged before coverage is evaluated. See [Evidence Sources](/concepts/evidence-sources/) for the full details on each source type.
...
# GitHub-flavored comment output (useful for CI comments)
supersigil verify --format github
```

The JSON format includes the full findings list, summary counts, optional evidence summary, and an optional `affected_summary` when you run `verify --since ...`.

## Affected Documents

When tracked files change, `supersigil verify --since <ref>` reports the associated specs as affected. Terminal output adds a short affected-doc hint, `--format github` renders an inline review dashboard for the affected specs, and JSON includes a compact `affected_summary`:

```bash
supersigil verify --since main
```

For a standalone affected-document check without full verification, use the `affected` command:
...
See the [CI Verification guide](/guides/ci-verification/) for complete GitHub Actions examples and practical policy for integrating verification into pull requests.
