...
Every Supersigil document produces nodes in the graph. The document itself is a node. Referenceable components inside it (`<Criterion>`, `<Task>`, `<Example>`) are also nodes. References between components form directed edges.
...
<Aside type="caution">
  All attributes must be plain string literals.
</Aside>
...
An executable code sample that can verify criteria. Contains a code block and an optional `<Expected>` child.

````mdx
<Example id="login-curl" lang="bash" runner="sh" verifies="auth/req/login#login-success">
...
- `verifies` creates evidence edges from the example to criteria.
- `runner` specifies which configured runner executes the code block.
...
:::tip
Use `supersigil plan --verbose` to see all criteria and full task details, not just the summary.
:::
