...
The [Quickstart](/supersigil/getting-started/) walks through a greenfield setup. This guide covers the more common case: you have working code, existing tests,
...
1. **Initialize Supersigil**
   ...
   This creates `supersigil.toml` and installs agent skills to `.agents/skills/`.
   The default `paths` glob (`specs/**/*.md`) is a good starting point — specs live in their own directory, separate from source code.
   Use `--no-skills` if you don't need the agent skills.
