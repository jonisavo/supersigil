...
<AcceptanceCriteria>
  <Criterion id="req-5-1">
    THE Parser SHALL extract PascalCase JSX elements from both Flow_JSX and
    Inline_JSX positions. Lowercase HTML or JSX elements SHALL NOT become
    supersigil components, but their children SHALL still be traversed.
...
<AcceptanceCriteria>
  <Criterion id="req-6-1">
    THE Parser SHALL validate extracted components against Component_Defs and
    emit `ParseError::UnknownComponent` for unknown PascalCase component names.
