...
6. Emit requirements, design, and tasks MDX documents for the files that were
...
The importer does not construct `SpecDocument` values directly. It emits MDX
strings and relies on downstream parser and graph layers to accept that output.
...
2. Write the planned MDX documents to a temp output tree.
...
- Building `SpecDocument` values directly instead of emitting MDX strings.
