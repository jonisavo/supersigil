//! Plugin assembly and evidence orchestration.
//!
//! Creates ecosystem plugin instances from config, runs plugin discovery,
//! and builds the merged artifact graph from plugin + explicit evidence.
...
///
/// `inputs` should be pre-resolved via [`VerifyInputs::resolve`] to avoid
/// redundant glob expansion and tag scanning.
///
