...
- `status_inconsistency` still exists as a configurable rule name, but the
  current built-in status rule is effectively a stub.
- Plugin failure and conflict findings are appended in CLI glue after the
  library run rather than being emitted from the verify crate itself.
