...
- `context` does not yet have CLI-level tests for every conditional section,
  especially `implemented_by`, `referenced_by`, and linked-task rendering.
- The evidence-aware filtering step for `plan` currently lives in CLI glue
  instead of the lower-level query model. That is a design choice, not a proven
  stable boundary.
- There is no end-to-end CLI test yet for the fully clean `plan` case that
  should print `No outstanding work.`
