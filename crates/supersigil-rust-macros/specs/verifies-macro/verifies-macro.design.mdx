...
<TrackedFiles paths="crates/supersigil-rust-macros/src/lib.rs, crates/supersigil-rust/tests/trybuild.rs, crates/supersigil-rust/tests/fixtures/pass/*.rs, crates/supersigil-rust/tests/fixtures/fail/*.rs, crates/supersigil-rust/tests/fixtures/validation-project/supersigil.toml, crates/supersigil-rust/tests/fixtures/validation-project/specs/test-req.mdx" />
...
The proc macro cannot depend on `supersigil-rust`, so it duplicates the
multi-project resolution logic that also exists in `scope.rs`.
...
- Cache invalidation currently keys only on `supersigil.toml` mtime, not on
  spec file mtimes.
- Multi-project scope resolution logic is duplicated between this crate and
