...
<TrackedFiles paths="crates/supersigil-rust-macros/src/lib.rs, crates/supersigil-core/src/rust_scope.rs, crates/supersigil-rust/tests/trybuild.rs, crates/supersigil-rust/tests/fixtures/pass/*.rs, crates/supersigil-rust/tests/fixtures/fail/*.rs, crates/supersigil-rust/tests/fixtures/validation-project/supersigil.toml, crates/supersigil-rust/tests/fixtures/validation-project/specs/test-req.mdx" />
...
The built graph is cached in a thread-local `Rc<DocumentGraph>` keyed only by
the `supersigil.toml` modified time.
...
- Cache invalidation currently keys only on `supersigil.toml` mtime, not on
  spec file mtimes.
