...
- Precomputed reverse mappings (who references/implements/depends-on what)
...
graph TD
    ...
    subgraph "GraphBuilder::build()"
        ...
        REV["8. Reverse Mappings<br/>(References/Implements/DependsOn)"]
...
pub struct DocumentGraph {
    ...
    /// Fragmentless References refs (e.g., `<References refs="auth/req/login" />`) are stored
...
impl DocumentGraph {
    ...
    /// - `fragment: Some("id")` → returns documents referencing that specific criterion.
    /// - `fragment: None` → returns documents that referenced the document without a fragment.
    /// To get ALL referencing documents (union of doc-level + all criterion-level),
...
pub struct ContextOutput {
    ...
    /// Criteria with their reference status
...
pub struct PlanOutput {
    /// Criteria with no verification evidence
...
1. **References reverse**: `(target_doc_id, Option<criterion_fragment>) → BTreeSet<referencing_doc_id>`. Fragmentless References refs (pointing at a document as a whole) are stored with `fragment: None`. They are preserved for traceability but do NOT satisfy `uncovered_criterion` coverage (which requires a specific criterion fragment).
2. **Implements reverse**: `target_doc_id → BTreeSet<implementing_doc_id>` (fragments discarded per design doc)
3. **DependsOn reverse**: `target_doc_id → BTreeSet<depending_doc_id>`
...
<References refs="document-graph/req#req-1-1, document-graph/req#req-1-4" />
...
<References refs="document-graph/req#req-1-2" />
...
<References refs="document-graph/req#req-2-1, document-graph/req#req-2-3, document-graph/req#req-2-4" />
...
<References refs="document-graph/req#req-2-2" />
...
<References refs="document-graph/req#req-3-2, document-graph/req#req-3-3, document-graph/req#req-3-4" />
...
<References refs="document-graph/req#req-3-5, document-graph/req#req-3-6, document-graph/req#req-3-7" />
...
<References refs="document-graph/req#req-4-1, document-graph/req#req-4-4" />
...
<References refs="document-graph/req#req-4-2, document-graph/req#req-4-3" />
...
<References refs="document-graph/req#req-5-2" />
...
<References refs="document-graph/req#req-5-3" />
...
<References refs="document-graph/req#req-5-4, document-graph/req#req-5-5" />
...
<References refs="document-graph/req#req-6-2" />
...
<References refs="document-graph/req#req-6-3" />
...
<References refs="document-graph/req#req-7-1, document-graph/req#req-7-2, document-graph/req#req-7-3" />
...
<References refs="document-graph/req#req-7-4" />

### Property 16: Reverse mapping completeness

*For any* document containing a `References`, `Implements`, or `DependsOn` component with resolved refs, the corresponding reverse mapping should contain the source document ID in the set for each target. For `Implements` and `DependsOn`, fragment portions are discarded — the mapping is document-level. Duplicate refs within the same attribute contribute only once.

<References refs="document-graph/req#req-8-1, document-graph/req#req-8-2, document-graph/req#req-8-3, document-graph/req#req-8-6" />

### Property 17: Context output completeness

*For any* valid document ID in the graph, the `ContextOutput` should contain: the target document's frontmatter and criteria, the reverse mappings of referencing/implementing documents (with status), and tasks from linked tasks documents in topological order.

<References refs="document-graph/req#req-9-1, document-graph/req#req-9-2, document-graph/req#req-9-3, document-graph/req#req-9-4, document-graph/req#req-9-5" />

### Property 18: Plan output correctness

*For any* valid document ID in the graph, the `PlanOutput` should contain: outstanding criteria (those with no completed task implementing them), pending tasks (status ≠ done) in topological order, and completed tasks with their implements refs.

<References refs="document-graph/req#req-10-1, document-graph/req#req-10-2, document-graph/req#req-10-3, document-graph/req#req-10-4" />
...
<References refs="document-graph/req#req-10-5" />
...
<References refs="document-graph/req#req-11-1, document-graph/req#req-11-2" />
...
<References refs="document-graph/req#req-12-1, document-graph/req#req-12-2, document-graph/req#req-12-4" />
...
<References refs="document-graph/req#req-13-1, document-graph/req#req-13-2, document-graph/req#req-13-3" />

### Property 23: Reverse mapping queryability

*For any* successfully built `DocumentGraph`, the reverse mapping accessors (`references`, `implements`, `depends_on`) should be queryable by target document ID or by `(document_id, fragment)` pair. Querying for a target that no document references should return an empty set (not an error).

<References refs="document-graph/req#req-8-4, document-graph/req#req-8-5" />
...
<References refs="document-graph/req#req-9-6" />
...
<References refs="document-graph/req#req-10-7" />
...
<References refs="document-graph/req#req-10-6" />
...
<References refs="document-graph/req#req-12-3" />
...
<References refs="document-graph/req#req-11-3" />
