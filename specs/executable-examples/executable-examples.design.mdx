...
An `Example` component contains a single fenced code block (the
executable code). An optional `Expected` child declares the golden
output with its own optional code block:
...
The parser extracts the code blocks and their byte offsets from within
the component bodies. The `Example` code block becomes `ExampleSpec.code`;
the `Expected` code block becomes `ExpectedSpec.body`.
...
| `supersigil-cli` | Two-phase verify orchestration including hook invocation. `examples` discovery command. `--skip-examples` and `--update-snapshots` flags on `verify`. |
...
The current `ExtractedComponent` stores `body_text: Option<String>` —
trimmed non-component text from the component body. Fenced code blocks
are flow content that the extractor currently skips.

For `Example` and `Expected`, the extractor needs to additionally
capture:

- **`code_blocks: Vec<CodeBlock>`** — fenced code blocks found in the
  component body, each with language tag and content string.

This is an additive change to `ExtractedComponent` in
`supersigil-core/src/types.rs`. The extractor in `supersigil-parser`
needs to be amended to populate these fields when the component name
is `Example` or `Expected`. All other components continue to get empty
vectors (zero allocation with `Vec::new()`).
...
single code block into `ExpectedSpec.body_span` as
`(content_offset, content_offset + content.len())`. This is the span
the snapshot rewriter uses in step 5 of the rewrite strategy.

#### Code block cardinality

- `Example` MUST contain exactly one fenced code block (the executable
  code). Zero or multiple code blocks is a lint error.
- `Expected` MUST contain at most one fenced code block (the golden
  output body). Zero is valid when only `status` or `contains`
  attribute checks are used. Multiple is a lint error.
...
1. Reads the source MDX file from disk.
2. Applies the same normalization (BOM strip, CRLF → LF).
3. Locates the `Expected` component's code block in the normalized
   text using the extracted byte offset.
...
If `lang` is omitted, the executor derives it from the first fenced
code block inside the example. That derived language is also what
`{lang}` interpolates in runner command templates.
...
child of `Example`, analogous to `check_verified_by_placement`.
...
pub struct ExampleSpec {
    ...
    /// Path to the source MDX file (for snapshot rewrites).
...
`Expected` code block in the source MDX file using the snapshot rewrite
...
pub struct ExamplesConfig {
    ...
    /// Maximum concurrent example executions. Default: 1.
...
| Example with zero or multiple code blocks | Lint error (caught at structural verify time) |
| Expected with multiple code blocks | Lint error (caught at structural verify time) |
...
### External snapshot storage instead of inline in MDX
...
the MDX file keeps everything co-located. The trade-off is that
