...
<AcceptanceCriteria>
  ...
  <Criterion id="req-3-3">
    ...
    it automatically using the `vscode-languageclient` library's built-in
    crash recovery.
    <VerifiedBy strategy="file-glob" paths="editors/vscode/src/extension.ts" />
  </Criterion>
...
<AcceptanceCriteria>
  ...
  <Criterion id="req-6-2">
    ...
    manifest, icon, readme, and runtime assets (grammars, preview
    styles) — no source files or `node_modules`.
    <VerifiedBy strategy="file-glob" paths="editors/vscode/.vscodeignore" />
  </Criterion>
  <Criterion id="req-6-3">
    ...
    <VerifiedBy strategy="file-glob" paths="editors/vscode/tsconfig.json, editors/vscode/esbuild.mjs" />
  </Criterion>
</AcceptanceCriteria>
```
